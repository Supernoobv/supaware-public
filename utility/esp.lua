...
local function updateObjectEsp()
    for object, esp in pairs(espObjectCache) do
        ...
        if getgenv().ROGUELINEAGE and getgenv().objectEsp then
            ...
            if #commontrinkets > 0 and table.find(commontrinkets, name) and not getgenv().COMMONTRINKETESP then
                shouldrender = false
            elseif #gems > 0 and table.find(gems, name) and not getgenv().GEMESP then
                shouldrender = false
            elseif #artifacts > 0 and table.find(artifacts, name) and not getgenv().ARTIFACTESP then
                shouldrender = false
            end

            if name == "Scroll" and not getgenv().SCROLLESP then
                shouldrender = false
            end
...
if getgenv().ROGUELINEAGE then
    ...
    for _,v in pairs(getgenv().COMMONTRINKETS) do
        print(v)
    end
end
